...
//!
//! Demonstrates how to set up a ClientHost that accepts incoming services RPC
...
//! services service can dial back into this endpoint to request diagnostics.
//!
...
async fn main() -> Result<()> {
    // 1. Create an endpoint that will both dial services and accept incoming
    //    requests from the services service via a ClientHost.
    ...
    //    with our project on services. This will create a capability token, send it to
    ...
    // 5. Set up a ClientHost so services can dial *back* into this endpoint.
    ...
    // 6. Run diagnostics locally (pass true to also upload results to services).
